...
# 실험 설명
# 학습할 때는 저 Goals 중 두 개를 선택해서 학습합니다.
# 학습이 끝나면 모든 Goals에 대해서 테스트합니다
...
def select_eval_goal():
    global goal_idx
    goal = GROUND_GOALS[goal_idx % 3]
    goal_idx += 1
    return goal
...
def cross_sparse(port1: int = 8001, port2: int = 8002, device_id: int = 0):
    group_name = f"cross-sparse-{TEST_GOAL_IDX}"
    run = wandb.init(
    ...
    eval_env = wrap_env(eval_base_env, size_x, size_y, select_eval_goal)
    ...
    eval_callback = EvalCallback(
        ...
        eval_freq=100,  # about every 20_0000 steps
        n_eval_episodes=5,
        deterministic=True,
        render=False,
    )
...
if __name__ == "__main__":
    ...
    cross_sparse(port1=port1, port2=port2, device_id=device_id)
