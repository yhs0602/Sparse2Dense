...
def new_dense(
    ...
    if base_checkpoint:
        if "sparse" in base_checkpoint:
            ...
            raise ValueError(f"Unknown base checkpoint: {base_checkpoint}")
        ...
        if "1000000" in base_checkpoint:
            ...
            timing_str = "unknown"
    group_name = (
        f"v50-room-new-dense-from_{from_str}-{entropy_coef}"  # {setting['spawn_idx']}
    )
    run = wandb.init(
    ...
    checkpoint_callback = CustomCheckpointCallback(
        ...
        save_path=f"models/{extended}/new-dense-from_{from_str}/{seed}",
        verbose=1,
    )
